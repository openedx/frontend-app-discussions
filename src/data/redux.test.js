...
import { blocksAPIResponse } from './__factories__';
...
describe('Course blocks data layer tests', () => {
  ...
  test('successfully processes block data', async () => {
    ...
    expect(store.getState().blocks.chapters)
      .toHaveLength(6);
    expect(store.getState().blocks.chapters.map(chapter => chapter.displayName))
      .toEqual(['Introduction', 'Example Week 1: Getting Started', 'Example Week 2: Get Interactive', 'Example Week 3: Be Social', 'About Exams and Certificates', 'holding section']);
    ...
    expect(store.getState().blocks.topics['98d8feb5971041a085512ae22b398613'])
      .toEqual({
        chapterName: 'Example Week 1: Getting Started', unitLink: 'http://localhost:18000/courses/course-v1:edX+DemoX+Demo_Course/jump_to/block-v1:edX+DemoX+Demo_Course+type@vertical+block@3dc16db8d14842e38324e95d4030b8a0', unitName: 'Videos on edX', verticalName: 'Lesson 1 - Getting Started',
      });
    Object.values(store.getState().blocks.topics).forEach(
      topic => {
